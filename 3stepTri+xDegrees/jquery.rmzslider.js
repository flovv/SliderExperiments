...
// <div class='button button_left'></div>
// <div class='handle handle_num_%N' id=class='handle handle_num_%N'></div>
//
		...
		var RmzSliderTemplate = function(target, params){
			...
			this.init = function(){
				...
				for (i = 0; i < P.numHandles; i++) {
					target.append("<div class='handle handle_num_"+i+"' id='handle handle_num_"+i+"'  style='display:none' ></div>");
				}
			...
			this.onContClick = function(event) {
				...
				if (P.countDisplayedHandles == 0){
				   document.getElementById('handle handle_num_0').style.display='block';
				   staging();
				   }
				   
				if (P.countDisplayedHandles == 1){
				document.getElementById('handle handle_num_2').style.display='block';
				 staging();
				 }
				
				if (P.countDisplayedHandles == 2){
				document.getElementById('handle handle_num_1').style.display='block';
				 staging();
				 }
				
				if (P.countDisplayedHandles < 3) {
				P.countDisplayedHandles++;
				...
				if ((clickx >= cleft) && (clickx <= cright)) {
					...
					if (pos >= P.width) pos = P.width;
					 ...
					 if (P.countDisplayedHandles == 1){
					target.find(".handle_num_"+0)  // move the first slider 
						.trigger("setvalue", {value: that._leftToValue(pos).value, animate: 1});
					
					target.find(".handle_num_"+1)  // and the middle slider!
						.trigger("setvalue", {value: that._leftToValue(pos).value+1, animate: 1});
					}
					if (P.countDisplayedHandles == 2)
					target.find(".handle_num_"+2)
						.trigger("setvalue", {value: that._leftToValue(pos).value, animate: 1});
			
					if (P.countDisplayedHandles == 3){
					  // get average of the other two and then set the middle!
					var v1 = that.values[0].value;// Number(document.getElementById('sliderb_input_0').value);
					var v2 = that.values[2].value; //Number(document.getElementById('sliderb_input_2').value);
						var newV = Math.round((v1+v2)/2);

						target.find(".handle_num_"+1)
						.trigger("setvalue", {value:newV,  animate: 1});
				 ...
				 if (P.countDisplayedHandles == 1)
					...
					var newV = Math.round((v1+v2)/2);
						...
						target.find(".handle_num_"+1)
						.trigger("setvalue", {value:newV, relative: 1, animate: 1});	
